...
const Results = ({ points, setCurrentQuestion, setPoints,explanation,wrong, setWrong } : any ) => {
    ...
    return (
        <div className="bg-gray-100 rounded-md md:w-2/3 mx-auto h-min shadow-md my-8 p-4 flex flex-wrap justify-evenly items-center ">
            <h1 className="text-center w-full font-semibold text-5xl my-4">Dosiahli ste <span className="block"><span className={` ${points > 7 && "text-green-500"}  ${points < 8 && "text-yellow-500"}  ${points < 4 && "text-red-500"} font-bold text-7xl`} >{points}</span> / 15 bodov</span></h1>
